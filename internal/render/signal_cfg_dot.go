...
func SignalCFGDOT(g *signal.SignalGraph, content map[string]*SignalFuncContent, title string, t Theme) string {
	...
	// Build forward adjacency from all BL edges for path finding.
	...
	// Find direct signal→signal edges.
	...
	for _, e := range g.Edges {
		...
		if !signalSet[e.From] || !signalSet[e.To] {
			continue
		}
		...
		edgeSeen[key] = true
		signalEdges = append(signalEdges, edgeInfo{e.From, e.To})
	}
		...
		for len(queue) > 0 {
			...
			for _, next := range fwd[cur] {
				...
				if signalSet[next] {
					...
					if !edgeSeen[key] {
						edgeSeen[key] = true
						signalEdges = append(signalEdges, edgeInfo{src, next})
					}
				} else {
